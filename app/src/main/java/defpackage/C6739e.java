...
/* renamed from: eؙۢۤ, reason: invalid class name and case insensitive filesystem */
/* loaded from: classes.dex */
public final class C6739e {
    public final float[] ad;

    public /* synthetic */ C6739e(float[] fArr) {
        this.ad = fArr;
    }
    ...
    public static final void adcel(float[] fArr, float f, float f2) {
        if (fArr.length < 16) {
            return;
        }
    ...
    public static final void appmetrica(float[] fArr) {
        if (fArr.length < 16) {
            return;
        }
        double d = 180.0f * 0.017453292519943295d;
        ...
        fArr[1] = (f * cos) - (f2 * sin);
        fArr[2] = (f2 * cos) + (f * sin);
        fArr[5] = (f3 * cos) - (f4 * sin);
        fArr[6] = (f4 * cos) + (f3 * sin);
        fArr[9] = (f5 * cos) - (f6 * sin);
        fArr[10] = (f6 * cos) + (f5 * sin);
        fArr[13] = (f7 * cos) - (f8 * sin);
        fArr[14] = (f8 * cos) + (f7 * sin);
    }

    public static final void billing(float[] fArr, float f, float f2) {
        if (fArr.length < 16) {
            return;
        }
        fArr[0] = fArr[0] * f;
    ...
    public static final void license(float[] fArr) {
        if (fArr.length < 16) {
            return;
        }
        fArr[0] = 1.0f;
    ...
    public static final void metrica(float[] fArr, C2359e c2359e) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        float f8 = fArr[13];
        float f9 = fArr[15];
        float f10 = c2359e.vip;
        float f11 = c2359e.metrica;
        float f12 = c2359e.license;
        float f13 = c2359e.appmetrica;
        float f14 = f3 * f10;
        float f15 = f6 * f11;
        ...
        c2359e.vip = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        c2359e.metrica = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        c2359e.license = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        c2359e.appmetrica = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }

    public static final void purchase(float[] fArr, float f) {
        if (fArr.length < 16) {
            return;
        }
    ...
    public static final void startapp(float[] fArr, float[] fArr2) {
    ...
    public static final long vip(long j, float[] fArr) {
        if (fArr.length < 16) {
            return j;
        }
        float f = fArr[0];
    ...
    public static /* synthetic */ void yandex(float[] fArr, float f, float f2, int i) {
        if ((i & 1) != 0) {
            f = 1.0f;
        }
        billing(fArr, f, f2);
    ...
    public final boolean equals(Object obj) {
        if (obj instanceof C6739e) {
            return AbstractC7890e.billing(this.ad, ((C6739e) obj).ad);
        }
        return false;
    }
    ...
    public final String toString() {
        ...
        sb.append(fArr[15]);
        sb.append("|\n        ");
        return AbstractC17540e.license(sb.toString());
    }
}
