package defpackage;

import org.conscrypt.BuildConfig;

/* compiled from: r8-map-id-a03018653d44f370ce7a671d31c0358eb3f30ef65367264f2a4c1bf16e5cfea3 */
/* renamed from: eٌۙۘ, reason: invalid class name and case insensitive filesystem */
/* loaded from: classes.dex */
public final class C8944e extends AbstractC11069e {
    @Override // defpackage.AbstractC11069e
    public final void billing(C17790e c17790e, Object obj) {
        C12058e c12058e = (C12058e) obj;
        c17790e.m4424case(c12058e.ad());
        String str = c12058e.f24166e;
        if (!AbstractC7890e.billing(str, BuildConfig.FLAVOR)) {
            AbstractC11069e.Signature.adcel(c17790e, 2, str);
        }
        int i = c12058e.f24167e;
        if (i != 0) {
            AbstractC11069e.billing.adcel(c17790e, 1, Integer.valueOf(i));
        }
    }

    @Override // defpackage.AbstractC11069e
    public final Object metrica(C17546e c17546e) {
        long license = c17546e.license();
        int i = 0;
        String str = BuildConfig.FLAVOR;
        while (true) {
            int billing = c17546e.billing();
            if (billing == -1) {
                return new C12058e(i, str, c17546e.appmetrica(license));
            }
            if (billing == 1) {
                AbstractC11069e.billing.getClass();
                i = c17546e.amazon();
            } else if (billing != 2) {
                c17546e.smaato(billing);
            } else {
                AbstractC11069e.Signature.getClass();
                str = c17546e.advert();
            }
        }
    }

    @Override // defpackage.AbstractC11069e
    public final int mopub(Object obj) {
        C12058e c12058e = (C12058e) obj;
        String str = c12058e.f24166e;
        int license = c12058e.ad().license();
        int i = c12058e.f24167e;
        if (i != 0) {
            license = AbstractC8647e.signatures(i, AbstractC11069e.billing, 1, license);
        }
        return !AbstractC7890e.billing(str, BuildConfig.FLAVOR) ? AbstractC11069e.Signature.advert(2, str) + license : license;
    }

    @Override // defpackage.AbstractC11069e
    public final void purchase(C0444e c0444e, Object obj) {
        C12058e c12058e = (C12058e) obj;
        String str = c12058e.f24166e;
        int i = c12058e.f24167e;
        if (i != 0) {
            AbstractC11069e.billing.startapp(c0444e, 1, Integer.valueOf(i));
        }
        if (!AbstractC7890e.billing(str, BuildConfig.FLAVOR)) {
            AbstractC11069e.Signature.startapp(c0444e, 2, str);
        }
        c0444e.m356try(c12058e.ad());
    }
}
